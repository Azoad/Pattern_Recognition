# Classification of samples using Baye's Theorem

import math

B = [0.80,0.91,0.93,0.95,1.32,1.53,1.57,1.63,1.67,1.74,
...
A = [3.54,3.88,4.24,4.30,4.30,4.70,4.75,4.97,5.21,5.42,
...
x = input("Please enter the sample: ")

count_A = 0
count_B = 0
for a, b in zip(A, B):
    if a>=math.floor(float(x)) and a<math.ceil(float(x)):
        count_A += 1
    if b>=math.floor(float(x)) and b<math.ceil(float(x)):
        count_B += 1

p_A = ((count_A/60)*0.5) / ((count_A/60)*0.5 + (count_B/60)*0.5)
p_B = 1 - p_A

if p_A >= p_B:
    print("The sample should be classified as class A")
else:
    print("The sample should be classified as class B")
